...
Generated offline from news sites by `scripts/news` (decided 2026-09-23):
- Race storylines, quotes, off-track context — scraped from The Race, RacingNews365, GPFans, PlanetF1
  (Crash.net: links only, it blocks AI crawlers) and summarized by Claude in the script, never at runtime.
  Every bullet links its sources; API data overrides any number in an article.
...
- paceRatings.json — GENERATED by `npm run pace` (scripts/pace; do not hand-edit). Stores each team's gap
  to the best team in % per round and dimension; src/lib/pace.ts turns gaps into 0–100 scores
  (1 pt per 0.1% for lap times, per 0.2% for speeds). `npm run pace` only adds new rounds;
  `-- 2026 --force` recomputes all (~4 min, OpenF1 rate limits), `--round N` one round.
...
- raceNews.json — GENERATED by `npm run news` (scripts/news; needs ANTHROPIC_API_KEY in .env.local, model
  claude-opus-5 with server-side refusal fallback). Per round: headline, bullets citing source ids, quotes
  verified verbatim against the article, source list, link-only headlines. `--dry-run` needs no key.
...
## Data Rules
- No AI/LLM calls at runtime (offline scripts may use Claude to summarize; the app only reads the saved JSON)
...
Phase 3 (pace pipeline + Pace Profiles tab).
